...
public interface WorkOrderRepository extends JpaRepository<WorkOrder, Long> {
    ...
    @Query("""
            ...
            where r.encounter.id = :encounterId
              ...
              and r.status not in :terminalStatuses
            """)
    List<WorkOrder> findActiveByEncounterAndToDepartment(Long encounterId, Long departmentId,
                                                               Collection<OrderStatus> terminalStatuses);
    ...
    @Query("""
            ...
            where r.encounter.id in :encounterIds
              and r.status not in :terminalStatuses
            """)
    List<WorkOrder> findActiveByEncounterIds(Collection<Long> encounterIds,
                                                   Collection<OrderStatus> terminalStatuses);
