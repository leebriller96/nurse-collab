...
class WorkOrderTest {
    ...
    void 생성하면_요청됨_상태이고_수행_파트는_검사_종류가_결정한다() {
        WorkOrder request = newRequest();

        assertThat(request.getStatus()).isEqualTo(OrderStatus.REQUESTED);
    ...
    void 검사실이_접수하면_예정시각이_기록된다() {
        ...
        request.transitionTo(OrderStatus.ACCEPTED, ActorSide.PERFORMER, null, scheduled);
    ...
    void 접수할_때_예정시각이_없으면_실패한다() {
        ...
        assertThat(errorOf(() -> request.transitionTo(
                OrderStatus.ACCEPTED, ActorSide.PERFORMER, null, null)))
    ...
    void 병동은_접수할_수_없다() {
        ...
        assertThat(errorOf(() -> request.transitionTo(OrderStatus.ACCEPTED,
    ...
    void 규칙표에_없는_전이는_거부된다() {
        ...
        assertThat(errorOf(() -> request.transitionTo(
                OrderStatus.IN_PROGRESS, ActorSide.PERFORMER, null, null)))
    ...
    void 보류하면_직전_상태를_기억한다() {
        ...
        assertThat(request.getHoldFromStatus()).isEqualTo(OrderStatus.ACCEPTED);
    ...
    void 보류할_때_사유가_없으면_실패한다() {
        ...
        assertThat(errorOf(() -> request.transitionTo(
                OrderStatus.ON_HOLD, ActorSide.PERFORMER, "   ", null)))
    ...
    void 보류를_해제하면_직전_상태로_돌아오고_사유가_지워진다() {
        WorkOrder request = accepted();
        request.transitionTo(OrderStatus.ON_HOLD, ActorSide.PERFORMER, "응급 환자 우선", null);
    ...
    void 보류_상태에서도_취소는_가능하다() {
        ...
        request.transitionTo(OrderStatus.CANCELLED, ActorSide.PERFORMER, "검사 취소", null);
    ...
    void 종료된_요청은_더_이상_바꿀_수_없다() {
        WorkOrder request = newRequest();
        request.transitionTo(OrderStatus.CANCELLED, ActorSide.REQUESTER, "환자 거부", null);

        assertThat(errorOf(() -> request.transitionTo(
                OrderStatus.ACCEPTED, ActorSide.PERFORMER, null, OffsetDateTime.now())))
    ...
    void 요청부터_완료까지_전체_흐름을_통과한다() {
        ...
        request.transitionTo(OrderStatus.IN_TRANSIT,  ActorSide.REQUESTER, null, null);
        request.transitionTo(OrderStatus.IN_PROGRESS, ActorSide.PERFORMER, null, null);
        request.transitionTo(OrderStatus.RETURNED,    ActorSide.PERFORMER, null, null);
        request.transitionTo(OrderStatus.COMPLETED,   ActorSide.REQUESTER, null, null);

        assertThat(request.getStatus()).isEqualTo(OrderStatus.COMPLETED);
        assertThat(request.getScheduledAt()).isEqualTo(scheduled);
    ...
    private WorkOrder accepted() {
        WorkOrder request = newRequest();
        request.transitionTo(OrderStatus.ACCEPTED, ActorSide.PERFORMER,
