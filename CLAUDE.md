# CLAUDE.md

> 이 파일은 저장소 루트에 `CLAUDE.md` 라는 이름으로 두면
> Claude Code 가 세션 시작 시 자동으로 읽는다.

---

## 프로젝트 개요

병원 파트 간 간호 협업 시스템 (nurse-collab).

병동과 다른 파트 사이의 **업무 요청**을 전화 대신 시스템으로 처리하고,
모든 과정을 기록으로 남기는 것이 목적이다.
이송(검사실)으로 시작했고 지금은 검체·약제·의공까지 같은 흐름으로 다룬다.

EMR 대체가 아니라 **EMR 옆에 붙는 협업 레이어**로 포지셔닝한다.

## 기술 스택

| 영역 | 스택 |
|---|---|
| 언어/프레임워크 | Java 21, Spring Boot 3.3 |
| 데이터 | PostgreSQL 16, Flyway, Spring Data JPA |
| 캐시 | Redis — 지금은 갱신 토큰 저장에만 쓴다 |
| 실시간 | Spring WebSocket + STOMP (내장 SimpleBroker) |
| 인증 | Spring Security + JWT |
| 프론트 | React 19, TypeScript, Vite, TanStack Query, Tailwind CSS 4 |
| PWA | vite-plugin-pwa — 홈 화면 설치. 실제 폰 설치는 HTTPS 가 전제다 |
| 테스트 | JUnit 5, AssertJ, Testcontainers |
| 빌드/배포 | Gradle, Docker, Docker Compose, Caddy, GitHub Actions |

아직 하지 않은 것. 스택표에 미리 적어 두면 있는 줄 알게 된다.

| 항목 | 상태 |
|---|---|
| shadcn/ui | 미사용. Tailwind 로 직접 만들었다 |
| Redis Pub/Sub | 미사용. 인스턴스를 여러 대로 늘릴 때 SimpleBroker 를 릴레이로 바꾼다 |

## 설계 문서

작업 전 반드시 참고할 것. `/docs` 에 있다.

| 파일 | 내용 |
|---|---|
| `docs/01-db-schema.md` | 테이블 13개 DDL, 설계 근거 |
| `docs/02-api-spec.md` | REST API 명세, 에러코드 체계, WebSocket 규격 |
| `docs/03-backend-structure.md` | 패키지 구조, 상태 전이 코드 |
| `docs/04-screens-permissions.md` | 화면 17개 정의, 권한 매트릭스 |
| `docs/05-deployment.md` | 배포 가이드. 서버·도메인 선택과 단계별 절차 |
| `docs/06-hospital-scale.md` | 병원 통합 확장. 원내/클라우드 분리와 업무 요청 일반화 |
| `docs/07-test-scenarios.md` | 손으로 따라 하는 전체 시나리오 테스트 케이스와 자동 검사 대응표 |

문서와 코드가 어긋나면 **문서를 먼저 고치고** 코드를 작성한다.

## 절대 규칙

### 1. 상태 전이 규칙은 `OrderType` 안에만 존재한다

서비스나 컨트롤러에 `if (status == ...)` 분기를 만들지 않는다.
새 상태나 전이가 필요하면 `OrderType.RULES` 의 종류별 표에만 추가한다.
상태 목록은 `OrderStatus` 에 모으고, **어디서 어디로 갈 수 있는가는 종류가 가진다.**
종류마다 흐름이 다르기 때문이다 — 검체는 이송하지 않고, 장비 수리는 환자가 없다.

### 2. 검증은 엔티티가 한다

`WorkOrder.transitionTo()` 안에서 검증하고 예외를 던진다.
서비스는 조회/저장/이벤트 발행만 담당한다.

### 3. 알림은 반드시 커밋 이후에 발송한다

`@TransactionalEventListener(phase = AFTER_COMMIT)` 만 사용한다.
서비스 안에서 `messagingTemplate.convertAndSend()` 를 직접 호출하지 않는다.

### 4. 환자 정보는 업무 응답에 싣지 않는다

업무 응답(`/work-orders`)에는 이름도 진단명도 주의사항도 없다.
환자는 `subjectRef` 라는 불투명한 UUID 로만 가리키고,
그것을 사람으로 되돌리는 것은 `/api/v1/phi` 뿐이다.
합치는 곳은 서버가 아니라 **브라우저**다. 서버에서 합치면 업무 쪽이
이름을 지나가며 보게 되고, 나눈 의미가 절반 날아간다.

원내에 닿지 못하면 빈칸으로 두지 않고 **"원내망에서만 조회됩니다"** 라고 말한다.
특히 확인 경고는 감추지 않는다 — "경고가 없다" 와 "경고를 못 받았다" 를
같게 보여주면 금기 환자를 그냥 검사실로 보내게 된다.

### 5. 환자 정보 접근은 소속이 아니라 관계로 판단한다

```java
// 금지
if (staff.getDepartment().getDeptType() == EXAM) return true;

// 필수
transferRequestRepository.existsByEncounterIdAndToDepartmentIdAndStatusNotIn(...)
```

### 6. 간호기록과 이송 이력은 삭제하지 않는다

`work_order_event`, `nursing_note` 는 append only.
DELETE 쿼리나 삭제 API 를 만들지 않는다.

### 7. 환자 데이터는 전부 가상 인물이다

실명, 실제 등록번호, 실제 병원명을 절대 넣지 않는다.
테스트 픽스처는 `김OO`, `P0001234` 형태로 만든다.

## 코딩 컨벤션

- 주석은 **한글**로 작성한다. "무엇을"이 아니라 **"왜"** 를 쓴다
- 엔티티에 `@Setter` 를 붙이지 않는다. 상태 변경은 의미 있는 메서드로 표현한다
  (`setStatus()` 금지, `transitionTo()` 사용)
- 생성자는 `protected` 로 막고 정적 팩토리 메서드(`create`, `of`)를 쓴다
- DTO 는 `record` 로 만든다
- 연관관계는 전부 `LAZY`
- 응답 DTO 를 엔티티로 직접 반환하지 않는다
- 테스트 메서드명은 한글로 쓴다 (`두_명이_동시에_접수하면_한_명은_실패한다`)

## 작업 방식

- 기능 단위로 작업하고, 끝날 때마다 테스트를 돌린다
- 상태 전이 관련 변경은 **반드시 단위 테스트를 먼저 작성**한다
- 통합 테스트는 Testcontainers 로 진짜 PostgreSQL 을 띄운다. H2 금지
  (JSONB, 파티셔닝 문법이 H2 에서 깨진다)

## 커밋 컨벤션

```
feat: 이송 요청 상태 전이 API 구현
fix: 보류 해제 시 직전 상태 복원 오류 수정
test: 동시 접수 시 낙관적 락 충돌 테스트 추가
docs: API 명세에 통계 엔드포인트 추가
refactor: 상태 전이 규칙을 enum 으로 이동
chore: Testcontainers 의존성 추가
```

한 커밋에 한 가지 목적만 담는다.

## 개발 진행 순서

1. 프로젝트 생성 + Flyway 스키마 (V1, V2)
2. 인증/권한 + 마스터 CRUD
3. **이송 요청 도메인 + 단위 테스트** ← 여기가 프로젝트의 핵심
4. 이송 API + 화면 (병동 W-01~05, 검사실 E-01~02)
5. WebSocket 실시간 연동
6. 동시성 처리 + 통계 화면
7. 간호기록 + 감사 로그

현재 위치: **화면 17개 + README · CI · 배포 구성, 업무 요청 일반화, 원내 서버 분리(한 빌드 세 역할 · 분리 배포)까지**

- 1단계: Gradle 프로젝트, Flyway, docker-compose, `global` 공통 기반.
- 2단계: JWT 인증, 파트·직원·검사종류, 마스터 조회 API.
- 3단계: 상태 전이 규칙(`OrderStatus`), 이송 요청 도메인, 요청번호 발번(V4).
- 4단계: 백엔드 API 전체 + 화면 7개(C-01 로그인, W-01 보드, W-02 환자상세, W-03 요청등록,
  W-04 현황, W-05/E-02 요청상세, E-01 큐). frontend/ 하위에 React 19 + Vite.
  실시간 갱신은 5단계에서 붙였다.
- 5단계: STOMP WebSocket. 요청 생성·상태 전이·메시지가 양쪽 파트 채널로 즉시 전파된다.
  알림은 AFTER_COMMIT 리스너에서만 나간다. 프론트는 알림으로 화면을 그리지 않고
  조회 캐시를 무효화해 REST 로 다시 받아온다. 폴링은 60초 보조 장치로만 남겼다.
- 6단계: 대기시간 통계 API 와 대시보드(A-01). 집계는 SQL 로 하고, 값은 전부 work_order_event 에서 나온다.
  일반 간호사는 403, 수간호사는 자기 파트가 관여한 요청만, 관리자는 전체를 본다.
  동시성 처리(낙관적 락)는 3단계에서 이미 끝났고 화면에서도 확인했다.
- 7단계(백엔드): 활력징후, 간호기록(SBAR), 감사 로그.
  간호기록은 삭제하지 않는다. 본인이 24시간 안에만 고칠 수 있고, 고치기 전 내용은
  별도 이력 테이블 없이 audit_log.detail 에 before/after 로 남는다.
  환자 정보를 열어본 것도 @Audited + AOP 로 자동 적재된다.
- 7단계(화면): 활력징후 입력(W-06), 간호기록(W-07), 접근 기록 조회(A-05).
  설계 문서의 우선순위 1 화면 10개와 우선순위 2~3 중 위 3개까지 만들었다.
- 알림함(C-02): 실시간 알림은 그 순간 화면을 보고 있어야 닿는다.
  폰을 주머니에 넣고 다니는 병동 간호사를 위해 나중에 확인할 곳을 둔다.
  요청에 관여하는 양쪽 파트에 남기고 행위자 본인은 뺀다.
  - **받는 사람은 그 요청에 손댄 사람이다**(요청자 + 진행 기록의 행위자). 한쪽에 아직 손댄 사람이 없으면
    그 파트 전원. 처음에는 매번 양쪽 파트 전원에게 보냈고, 반년치 데이터로 세니 하루 7만 5천 건이었다.
    남의 환자 알림까지 쌓이는 알림함은 아무도 열지 않는다. 파트 전체 소식은 실시간 채널이 맡는다.
  - 알림은 이력의 복사본이라 지운다(읽은 것 30일, 안 읽은 것 90일). 지워도 `work_order_event` 에 다 남는다.
  - **접수 지연은 올린다.** 응급 10분·긴급 30분 동안 아무도 접수하지 않으면 양쪽 수간호사와 요청자에게
    한 번 알리고 파트 채널에 `ORDER_DELAYED` 를 방송한다. 아무도 누르지 않은 요청은 큐에 조용히 남기 때문이다.
    - 표시는 `delay_notified_at` 한 칸이고 **`version` 을 올리지 않는다.** 올리면 알림이 나가는 그 순간 접수를 누른
      간호사가 "다른 사용자가 먼저 처리했습니다" 를 받는다. 그래서 엔티티를 거치지 않고 조건부 일괄 갱신으로 찍는다 —
      갱신된 행이 1일 때만 알리므로 서버가 여러 대여도 한 번만 나간다.
    - 12시간 넘은 요청은 올리지 않는다. 오래 꺼져 있던 서버가 켜지며 한꺼번에 울리는 것을 막는다.
- 추가 완료: 지난 요청 조회(E-04), 기준 정보 관리(A-02~A-04), 일정 보드(E-03).
  E-04 는 새 엔드포인트를 만들지 않고 기존 목록에 기간과 검색어를 열어 처리한다.
  마스터는 어느 것도 삭제하지 않는다. 지난 요청 이력이 참조하고 있기 때문이다.
- 마무리: README, GitHub Actions CI, 배포 구성.
  - CI 는 백엔드(Testcontainers 포함)·프론트·도커 이미지 세 잡이다.
    이미지 잡은 앞의 둘이 통과한 뒤에만 돈다.
  - 배포는 `docker-compose.prod.yml` 로 이미지 두 개(백엔드 JAR, Caddy+정적파일)와
    DB·Redis 를 올린다. 밖에서 닿는 것은 웹뿐이고 나머지는 내부 네트워크에만 붙는다.
  - Caddy 가 같은 오리진에서 `/api` 와 `/ws` 를 중계한다. 개발 때 Vite 프록시가 하던 것과 같은 모양이라
    프론트 코드 어디에도 서버 주소가 없다.
  - `SITE_ADDRESS` 에 도메인을 넣으면 인증서 발급·갱신이 자동이다. 비우면 `:80` 평문으로 뜬다.
    nginx+certbot 대신 Caddy 를 고른 이유는 **갱신이 조용히 실패하는 것**이 이 구성에서
    제일 무서운 고장이기 때문이다.
  - HTTPS 는 선택이 아니다. 폰 브라우저가 `http://` 페이지에서 `wss://` 를 막아
    평문으로 공개하면 실시간 갱신이 조용히 죽는다. PWA 도 HTTPS 가 전제다.
  - `prod` 프로파일에서만 `forward-headers-strategy` 를 켠다. 켜지 않으면 감사 로그의 IP 가
    전부 웹 컨테이너가 되고, 반대로 백엔드가 직접 열려 있는 로컬에서 켜면 헤더를 위조당한다.
  - README 스크린샷은 `e2e/screenshots.mjs` 가 뽑는다. 리허설 녹화와 분리한 이유는
    영상에 덮어쓰는 자막이 스틸에 그대로 찍히기 때문이다.
- 데모 초기화(`ops/demo-reset/`): 공개 배포는 데모 계정 비밀번호가 공개돼 있어
  누구나 데이터를 바꾼다. 매일 새벽 되돌린다. 로컬 시연 데이터도 같은 스크립트로 심는다.
  - **초기화가 심는 시드는 `ops/demo-reset/seed/` 에 둔다.** 처음에는 Flyway 시드 파일을
    그대로 다시 돌렸는데, 시드는 "그때의 스키마" 를 향해 쓰여 있다. V8 에서
    `exam_type` 을 `service_item` 으로 바꾸자 V3 이 없는 테이블에 INSERT 하게 됐고,
    이미 적용된 마이그레이션이라 고칠 수도 없다(체크섬이 깨진다).
    두 곳이 어긋나는 것은 CI 가 잡는다. 초기화를 돌린 뒤 브라우저 검사가
    거기 심은 계정으로 로그인하고 거기 심은 업무 항목을 고른다.
  - 전이 경로도 적어 두지 않는다. 종류마다 흐름이 다른데 그 표를 베끼면 서버 규칙을
    고칠 때 시연 데이터만 조용히 어긋난다. **서버가 내려주는 `availableTransitions` 를 따라 걷는다.**
  - **지킬 목록을 만들지 않는다.** 전부 비우고 시드를 처음부터 다시 넣는다.
    방문자가 `admin01` 을 간호사로 강등시키면 목록 방식으로는 그 상태가 영구히 남아
    아무도 관리자 화면에 못 들어간다(실제로 재현해 확인했다). 시드에 행을 추가할 때
    목록을 같이 고쳐야 하는 문제도 없어진다.
  - 요청은 SQL 이 아니라 실제 API 로 만든다. 직접 꽂으면 `work_order_event` 가
    비어서 통계·이력 화면이 텅 빈다.
  - **시드가 `seed/work` 와 `seed/phi` 로 나뉜다.** 두 서버로 가르면 DB 도 둘이라 한쪽 시드가
    다른 쪽 테이블을 조인할 수 없다. 원내 시드는 병동 id·기록자 id 를 초기화 스크립트가
    업무 DB 에서 읽어 psql 변수로 넘긴다. 원내 서버가 소속과 직원을 토큰의 id 로만 아는 것과 같다.
    - 가명은 고정값이다. `phi/01-patients.sql` 과 `work/02-care-episodes.sql` 이 **같은 가명·병실·병상**을
      들고 있어야 한다. 예전에는 `encounter` 에서 복사했는데 DB 가 갈리면 그렇게 할 수 없다.
    - `PHI_PG*` 를 주지 않으면 둘 다 한 DB 에 넣는다 — 한 서버로 띄울 때다.
  - 시각만 SQL 로 되돌린다. `now()` 기준이어야 한다. 고정 시각에 맞추면 그 전에 돌렸을 때
    요청이 미래로 가고 대기시간이 전부 0분으로 찍힌다.
  - **작은 병원 반년치(`SEED_PROFILE=hospital`, `seed-hospital/`).** 요청 10만 건은 API 로 만들 수 없어
    SQL 로 넣는다. 위의 "요청은 API 로" 원칙을 어기는 대신 서버가 남기는 모양을 전부 같이 만든다 —
    진행 기록, `version` = 전이 횟수, 요청번호와 `request_no_sequence`, 보류의 `hold_from_status`.
    전이 순서는 `OrderType` 규칙표를 따르므로 **규칙표를 고치면 `work/03-orders.sql` 도 고쳐야 한다.**
    - `random()` 을 쓰지 않는다. 두 DB 를 따로 채우므로 `common/00-plan.sql` 을 양쪽 파일 앞에 붙여
      같은 계산을 각자 다시 하고, 값은 전부 md5 해시에서 나온다. 기준 시각은 스크립트가 한 번 정해 넘긴다.
    - 메시지 내용만은 업무 DB 를 읽은 뒤에 원내에 채운다(`bridge/`). 누가 언제 보냈는지가 업무 쪽에서 정해진다.
    - 최근 하루치 요청 일부는 일부러 중간에서 멈춘다. 없으면 새벽에 심었을 때 큐가 전부 비어 보인다.
      이 데이터를 자정에 심어 보고 목록 조회가 "오늘" 로만 거르던 결함(자정에 안 끝난 요청이 큐에서
      사라짐)을 찾았다. 기본 데이터 20건으로는 드러나지 않는 것들이 있다 — 응급 정렬도 그랬다.
    - **그래서 CI 가 마지막에 병원 데이터로 갈아 끼운다**(`e2e/check-hospital-data.mjs`). 브라우저 없이 응답의
      순서(응급 먼저)·개수(기간 없는 큐 = 반년을 열어 센 안 끝난 요청)·시간을 보고, 병동 간호사 알림함이 넘치지 않는지,
      업무 DB 의 재원이 원내 DB 에서 전부 사람으로 풀리는지 본다. 개수는 초기화 시각에 따라 달라지므로
      "몇 건 이상" 이 아니라 두 조회가 같은지를 건다.
- 폰에서 쓸 수 있게(PWA·오프라인·오류 차단):
  - **API 응답은 절대 캐시하지 않는다.** 앱 껍데기만 캐시한다. 오프라인에서 지난 활력징후나
    낡은 이송 상태를 지금 값처럼 보여주면 이미 끝난 검사를 다시 보내게 된다.
    모르는 것보다 틀리게 아는 것이 나쁘다. 그래서 끊기면 값을 내놓는 대신 끊겼다고 알린다.
  - 오프라인 배너는 `fixed` 가 아니라 `sticky` 다. `fixed` 면 검사실·관리자 화면의
    상단 메뉴를 덮어 끊긴 동안 로그아웃도 탭 이동도 못 한다.
  - `ErrorBoundary` 가 없으면 렌더링 오류 하나에 화면이 백지가 된다. 근무 중에 그러면
    간호사는 결국 전화기를 든다. 이 프로젝트가 없애려던 그 전화다.
  - 개발 서버에서는 서비스 워커를 끈다. 캐시가 HMR 과 얽히면 고친 코드가 왜 안 나오는지
    한참 찾게 된다. 확인은 `npm run build && npm run preview` 후 `e2e/check-pwa.mjs` 로 한다.
  - 내장 브라우저 패널은 서비스 워커를 막는다. 확인은 Playwright 로 해야 한다.
- 감사 로그 파티션은 `ensure_audit_log_partitions()` 가 하루 한 번 채운다(V7).
  손으로 만든 것은 2027년 3월에서 끝나는데, 그 뒤로는 행이 기본 파티션으로 들어가고
  **INSERT 는 성공하므로 아무도 모른다.** 파티션을 나눈 의미만 조용히 사라진다.
  - DDL 은 Flyway 가 만든 함수 안에 둔다. 스키마의 모양을 아는 곳이 두 군데면 안 된다.
  - 실패해도 기동을 막지 않는다. 기본 파티션이 받아 주므로 기록은 계속 남는다.
    감사 때문에 진료가 멈추면 안 된다는 원칙과 같다.
  - `audit_log_default` 에 행이 있으면 이 작업이 밀렸다는 뜻이다. 경고를 남긴다.
- 업무 요청 일반화: 이송 하나였던 흐름을 네 종류로 넓혔다(V8 이름, V9 종류, V10 시드).
  부서마다 도메인을 새로 만들면 얕은 CRUD 가 부서 수만큼 늘어난다. 추상을 한 단계 올렸다.
  - **전이 규칙은 `OrderType` 안에만 있다.** 상태 목록만 `OrderStatus` 에 모은다.
    DB 에 문자열 한 컬럼으로 저장되고 감사 로그와 통계가 그 값을 읽으므로,
    종류별로 같은 이름을 따로 두면 `IN_PROGRESS` 가 어느 것인지 알 수 없어진다.
  - 의공(장비 수리)을 넣은 이유는 **이것만 환자가 없기** 때문이다. 셋 다 "환자가 있고
    병동이 건다" 면 일반화가 됐는지 알 수 없다. `encounter_id` 가 NULL 이 되면서
    재원 정보를 그냥 꺼내 쓰던 자리와, inner join 때문에 목록에서 통째로 빠지던 것이 드러났다.
  - 환자를 붙일 수 없는 업무에 붙이는 것은 막는다(ORD-007). 환자 정보 접근을
    "진행 중인 요청이 걸려 있는가" 로 판정하므로, 장비 수리에 환자를 매달면
    의공학팀이 그 환자의 활력징후와 간호기록까지 열게 된다.
  - 화면은 규칙표를 들지 않는다. 버튼의 이름·행동·필수 입력을 서버가 함께 내려준다
    (`TransitionOption`). 화면이 표를 들면 종류를 더할 때 두 곳을 고쳐야 하고,
    한쪽만 고쳐진 날 사유 칸이 뜨지 않은 채 버튼만 누르게 된다.
  - 업무 항목 정렬은 `OrderType` 선언 순서를 따른다. 코드 알파벳순이면 `BME_*` 가
    맨 앞에 와서 요청 화면의 첫 탭이 의공이 된다. 무엇이 주된 업무인지는 철자가 정하지 않는다.
  - 홈 화면 판정은 `homeFor` 한 곳에만 둔다. 로그인 직후와 "/" 를 열었을 때 두 군데에
    같은 판정이 있었고, 부서 유형을 늘렸을 때 한쪽만 고쳐져 의공학팀 계정이
    자기 일이 하나도 없는 병동 보드로 떨어졌다.
  - 규칙표에 거는 불변식을 종류 전체에 건다(`OrderTypeTest`). 특히 **모든 상태에서
    빠져나갈 길이 있는가** — 막다른 상태에 빠진 요청은 버튼이 하나도 없고 목록에 영원히 남는다.
- 진료정보와 업무정보를 갈랐다(V11). 업무 쪽 테이블 중 진료 쪽을 참조하는 것이 하나도 없다.
  끊어 두었으므로 진료 쪽을 원내 DB 로 통째로 옮길 수 있다.
  - 가명은 **사람이 아니라 재원 건**에 붙는다. 같은 사람이 3년 뒤 다시 입원하면
    다른 열쇠를 받는다. 사람에 붙이면 업무 데이터만 보고도 "이 사람이 네 번 입원했다" 를
    알 수 있고, 그건 가명정보라고 부르기 어렵다.
  - 병실·병상은 업무 쪽(`care_episode`)에 둔다. 침대는 사람을 가리키지 않는다.
    이것까지 원내로 보내면 원내망 밖에서 병동 보드가 통째로 비어 업무가 안 돌아간다.
  - 반대로 진단명과 거동 여부는 업무 쪽으로 넘기지 않는다. 이송 준비물을 정하는
    값이더라도 그건 그 사람의 건강 상태다. 기준을 한 번 느슨하게 하면 다음 회색지대는
    더 쉽게 넘어간다.
  - **양쪽이 서로를 외래키로 잇지 않는다**(V11, V13). 두 DB 로 갈라 놓아야 하기 때문이다.
    이름이 필요한 곳은 둘로 나뉜다 — 기록한 사람의 이름은 쓸 때 굳히고(그때 그 사람이어야 한다),
    병동 이름은 굳히지 않는다(전동하면 바뀌는 값이라 굳히면 어긋난다).
  - **원내 코드는 업무 쪽 저장소를 주입받지 않는다.** 묻는 것은 `WorkRelationPort` 하나다 —
    "이 대상에 우리 파트로 온 진행중 요청이 있는가", "침대가 찼다". 한 서버일 때는
    `LocalWorkRelationAdapter` 가 같은 프로세스에서 답한다. 여기에 메서드가 늘기 시작하면
    경계가 새고 있다는 뜻이다.
    - `architecture/PhiBoundaryTest`(ArchUnit)가 양방향을 지킨다. 공유해도 되는 것은
      값의 이름(`StaffRole`, `DeptType`, `AlertType`, `EncounterStatus`)뿐이다.
      한 프로세스인 동안에는 저장소를 직접 주입해도 멀쩡히 돌아서, 갈라 띄우는 날까지 아무도 모른다.
    - 기록자는 DB 에서 다시 읽지 않고 토큰의 `staffId`·`name` 을 쓴다. 직원 정보는 업무 쪽 DB 에 있다.
    - `app.work-api.base-url` 을 채우면 `HttpWorkRelationAdapter` 가 업무 서버의
      `/work-relations` 에 **사용자 토큰을 그대로 실어** 묻는다. 서버용 자격증명을 두지 않는 것은
      그것 하나로 모든 파트에 대해 물을 수 있게 되기 때문이다. 닿지 못하면 `503 PHI-002` —
      "관계 없음" 으로 치면 끊긴 것이 권한 문제로 보인다.
    - 켜고 끄는 조건에 `@ConditionalOnProperty` 를 쓰지 않는다. 그 조건은 빈 문자열도 "있음" 으로
      쳐서, 주소를 비워 두면 HTTP 어댑터가 빈 주소로 뜬다. `@ConditionalOnExpression` 으로 비었는지 본다.
  - **한 빌드, 세 역할.** `app.role` = `combined`(기본) / `cloud` / `onprem`. 프로파일로 켠다.
    - 어느 쪽 코드인지는 패키지로만 정하고 `global/config/AppBoundary` 에 적는다. 컴포넌트·저장소
      스캔(`RoleTypeExcludeFilter`)과 엔티티 목록(`JpaConfig`), 경계 테스트가 **같은 표**를 본다.
      새 `domain` 패키지를 만들면 이 표에 넣어야 한다. 안 넣으면 테스트가 막는다 —
      분류되지 않은 코드는 양쪽 모두에 올라가기 때문이다.
    - 역할 DB 는 V14 까지 같은 이력을 쓰고, V15 가 상대 쪽 테이블을 걷어낸다(`db/split/*`).
      V15 는 방금 만든 DB 에서만 돈다 — 쓰던 합친 DB 에 `cloud` 를 붙이면 환자 테이블이 지워진다.
    - 한쪽 테이블만 건드리는 새 마이그레이션은 `db/migration` 에 두지 않는다. 상대 역할 DB 에는
      그 테이블이 없어 실패한다. 그런 것이 처음 생길 때 `db/phi`·`db/work` 를 만들고 위치에 더한다.
    - `RoleStartupTest` 가 세 역할을 실제로 띄워 빈과 테이블을 확인하고,
      합친 스키마가 두 역할 스키마의 합과 같은지 본다.
  - **원내 분리 배포는 `docker-compose.split.yml`** 이다. 같은 이미지를 `prod,cloud` 와 `prod,onprem` 으로 띄운다.
    - 망이 둘이고 업무 서버는 원내 망(`phi`, internal)에 붙지 않는다. 원내 서버와 중계 서버만 두 망에 붙는다.
    - 원내 서버에는 공개키 **파일 하나만** 붙인다. 디렉터리를 붙이면 쓰지 않아도 개인키 파일이 따라 들어간다.
    - Caddy 는 `/api/v1/phi/*` 만 `PHI_UPSTREAM` 으로 보낸다. 비어 있으면 백엔드로 가므로
      한 서버 구성과 Caddyfile 이 같다.
    - CI 는 이 구성으로 스택을 올리고, 브라우저 검사 전에 **격리를 직접 확인한다**(업무 서버에서
      `phi-postgres` 이름이 풀리지 않는지, 원내 서버에 개인키 파일이 없는지). 망 하나를 잘못 붙이면
      화면은 전부 멀쩡한 채로 격리만 사라진다.
    - 한 서버 구성(`docker-compose.prod.yml`)은 남긴다. 2GB 한 대로 공개 데모를 올리는 길이다.
      CI 는 그 파일이 올바른지(`config -q`)만 본다.
    - 원내 경로는 허용 대역(`PHI_ALLOWED_NETS`, 분리 구성 기본은 사설망)에서만 받는다. 밖에서 오면 Caddy 가
      **403 이 아니라 연결을 끊는다**(`abort`). 화면은 응답 없는 실패를 "원내망에서만 조회됩니다" 로 읽기 때문이다.
      CI 는 허용 대역을 어디에도 맞지 않는 값으로 바꿔 중계 서버만 다시 띄우고 `check-phi-gateway` 로 확인한다.
  - 입원 등록은 `AdmissionService` 한 곳에서만 두 곳에 쓴다.
    진료 쪽을 떼어내면 이 자리가 **원내에서 밖으로 나가는 유일한 통로**가 된다.
    통로가 하나여야 무엇이 나가는지 한 곳만 보면 알 수 있다.
  - **대화 내용도 원내다**(`request_message_body`). 업무 쪽 `request_message` 에는 누가·언제와
    `message_ref` 만 남는다. 9장에서 정해 두고 한동안 옮기지 않았던 것이다.
    - 쓰기는 원내가 받아 저장한 뒤 업무 서버에 알린다. **관여하는 파트인지는 업무 서버가 판정한다** —
      요청 당사자를 아는 곳이 거기다. 거절되면 원내 저장도 되돌린다.
    - 읽기도 원내가 화면이 들고 온 `message_ref` 를 믿지 않는다. 업무 서버에 읽을 수 있는 목록을 물어
      그 본문만 돌려준다. 열쇠를 손에 넣는 것만으로 열리면 안 된다.
    - 원내에 못 닿으면 목록에 건수는 보이고 내용 자리에 "원내망에서만 조회됩니다" 가 뜬다. 보내기는 막는다.
  - 알림 문구와 실시간 방송에도 이름을 싣지 않는다. 알림은 DB 에 쌓이고 폰 알림창에도 뜨며,
    실시간 채널을 구독하는 브라우저가 원내망 밖에 있을 수도 있다. 침대 번호로 충분하다.
  - 이름 검색은 두 걸음이다. 원내에 이름을 물어 가명을 받고, 그것으로 업무 목록을 거른다.
    원내에 닿지 못하면 이름 검색만 안 되고 요청번호 검색은 그대로 된다.
  - 원내는 자기 접근 기록(`phi_access_log`)을 따로 남긴다. 클라우드의 `audit_log` 는
    클라우드가 뚫리면 같이 지워지거나 고쳐질 수 있다. 진료정보가 실제로 나간 곳은
    원내이므로 그 기록도 원내에 있어야 한다.
    - **거절된 시도도 남긴다.** 업무 쪽 감사 AOP 는 성공한 요청만 적는데,
      조사할 때 제일 보고 싶은 것은 "누가 볼 수 없는 것을 열려고 했는가" 다.
    - 거절은 예외를 던져 본 트랜잭션이 롤백되므로 `REQUIRES_NEW` 로 따로 적는다.
      안 그러면 제일 남겨야 할 기록이 제일 먼저 사라진다.
    - 직원 정보에 외래키를 걸지 않고 아이디를 문자열로 함께 적는다.
      업무 쪽 DB 가 없어도 이 로그만으로 읽을 수 있어야 한다.
    - A-05 화면이 원내 `/phi/access-logs` 로 읽는다. 처음에는 "보여주려면 클라우드를 거쳐야
      하니 화면에 띄우지 않는다" 고 적었는데 틀린 판단이었다. 브라우저가 원내에 직접 물으면
      클라우드는 이 기록을 보지 않는다. 원내망 밖에서 화면이 열리지 않을 뿐이다.
    - **원내 기록은 원내에만 남긴다.** 간호기록·활력징후·주의사항의 감사와 간호기록 수정 전후를
      업무 쪽 `audit_log` 에서 여기로 옮겼다(V14 `detail`). 수정 전 내용은 그 자체가 진료정보다.
      7단계 설명의 `@Audited` · `audit_log.detail` 은 그때의 구조다.
    - 업무 쪽 `audit_log` 에는 로그인·로그인 실패·로그아웃과 기준 정보 변경이 남는다. 로그인 실패는
      화면에 같은 `AUTH-001` 을 주지만 기록에는 없는 아이디·틀린 비밀번호·비활성을 구별해 남긴다.
      직원 수정은 전후까지 남긴다 — 역할을 누가 바꿨는지가 첫 질문이다. 비밀번호는 어디에도 남기지 않는다.
    - 쓰기는 `saveAndFlush` 로 확정한 뒤 기록한다. 기록은 `REQUIRES_NEW` 라 먼저 커밋되는데,
      저장이 실패했는데 "기록했다" 가 남으면 기록을 믿을 수 없다.
  - 조회량 제한: 최근 10분에 서로 다른 30명을 넘기면 막는다(`PHI-001`).
    클라우드가 뚫리면 토큰을 새로 발급해 원내에 물어볼 수 있다. 인증 서버가
    클라우드인 이상 이 구멍은 구조적으로 남는다 — **막지는 못하고 좁히고 남긴다.**
    - 요청 수가 아니라 서로 다른 사람 수를 센다. 같은 환자를 다섯 번 여는 것은 정상이고
      스무 명을 한 번씩 여는 것이 이상한 일이다.
    - 목록 이름 채우기(`/phi/subjects/brief`)는 세지 않는다. 그건 "여는 것" 이 아니고,
      볼 자격이 없는 가명은 애초에 빠지므로 이 통로로는 관계 밖의 사람을 긁어갈 수 없다.
    - 관리자도 예외로 두지 않는다. 관리자 계정이야말로 노리는 쪽이 제일 갖고 싶어 하는 것이다.
  - **환자 조회는 전부 `/phi` 아래로 모았다.** 예전에는 병동 보드가 `/encounters` 로
    재원 목록을 받았고 그 응답에 이름과 진단명이 실려 있었다. 그래서 원내망 밖에서
    이 화면만 이름이 그대로 보였다 — 다른 화면은 사라지는데. 어디까지가 원내인지
    화면마다 다르면 아무도 기억하지 못한다.
    - 병동 보드는 이제 `/care-episodes`(침대·요청 수)와 `/phi/subjects/brief`(사람)를
      합쳐 그린다. 원내가 끊기면 "302-1에 누군가 있고 요청 2건" 까지는 보인다.
    - 활력징후·간호기록 화면은 통째로 진료 기록이라 일부만 비는 것이 아니다.
      빈 목록을 보여주면 "기록이 없다" 로 읽히므로 화면 대신 이유를 띄운다.
    - 주소에서도 재원 id 를 걷어냈다(`/ward/subjects/{가명}`).
      브라우저 기록과 중계 서버 로그에 남는 것이 가명이어야 한다.
    - 활력징후·간호기록 API 도 `/phi/subjects/{가명}/..` 이다. **원내 것은 경로가 `/phi` 로
      시작한다** — 두 서버로 가르면 중계 서버가 이 앞머리 하나만 보고 보낼 곳을 정한다.
      새 원내 경로를 `/phi` 밖에 만들면 그 요청은 조용히 클라우드로 간다.
    - 목록에 나가는 것은 보는 사람에 따라 다르다. 담당 병동은 진단명과 주의사항
      뱃지까지, 검사실은 이름과 "주의할 것이 몇 건" 까지. 검사실 상세에 진단명이
      실리지 않는 것과 같은 규칙이다.
  - `e2e/check-phi-split.mjs` 가 일부러 원내를 끊어 본다. 원내가 살아 있으면 화면은
    늘 멀쩡해 보여서, 끊어 봐야 무엇이 어디서 오는지 드러난다.
    업무 응답 본문을 직접 뒤져 이름·진단명이 섞여 있지 않은지도 함께 본다.
- 환자 주의사항(W-02)은 간호사가 남긴다. 낙상 위험, 폐소공포 이력, 격리처럼
  EMR 의 진단명이 아니라 **곁에서 본 것**이기 때문이다.
  - 경로가 재원이 아니라 환자다. 퇴원한다고 인공관절이 사라지지 않는다.
    접근 판정만 재원 기준이라 `requireViewableByPatient` 로 잇는다.
    판정을 다시 구현하면 한쪽만 고쳐지는 날이 오고, 그때 남의 병동 환자에게 붙일 수 있게 된다.
  - 지우지 않고 내린다. 이 주의사항을 보고 판단한 지난 요청이 있다.
  - 위험도는 화면에서 "참고 / 주의 / 중대" 로 푼다. INFO/WARN/CRITICAL 로는 무엇을 고를지 모른다.
  - 검사실도 요청이 걸려 있는 동안은 남길 수 있다. "검사대에 눕히니 폐소공포 반응" 은
    검사실에서만 알 수 있다.
- **목록은 누구 차례인지 알려준다**(`myTurn`). 보는 쪽이 다음 걸음을 눌러야 하면 참이다. 보류·취소는 치지 않는다 —
  요청됨에서 병동이 취소를 누를 수 있다고 "병동 차례" 로 치면 접수를 기다리는 요청이 전부 병동이 멈춰 세운 것처럼 보인다.
  - W-04 가 "우리 차례" 와 "기다리는 중" 으로 나뉜다. 교대 때 넘기는 말이 다르다(수령 확인 안 한 약, 결과 안 본 검체).
    인수인계 화면을 따로 만들지 않았다 — 자정 결함을 고친 뒤 W-04 가 이미 날짜를 넘어 안 끝난 요청을 들고 있었고,
    빠진 것은 "누구 차례인가" 하나였다.
  - E-01 은 병동 차례인 행을 흐리게 하고 오래 기다려도 빨갛게 칠하지 않는다. 검사실이 할 수 없는 일에 눈이 먼저 가면 안 된다.
  - 판정은 `OrderType.isTurnOf` 에만 있다. `OrderTypeTest` 가 "안 끝난 상태에는 정확히 한쪽의 차례가 있다" 를 모든 종류에 건다.
- **폰 알림(Web Push).** 알림함에 쌓이는 그 알림을 폰 알림창에도 띄운다. 받는 사람·문구는 알림함 규칙 그대로다.
  - 라이브러리를 쓰지 않았다. 널리 쓰이는 자바 구현이 몇 년째 멈춰 있고 낡은 BouncyCastle 을 끌고 온다.
    RFC 8291 암호화는 JDK 로 충분하고, **RFC 부록의 시험값에 바이트 단위로 맞춰 둔다**(`WebPushEncryptionTest`).
    직접 짠 암호는 그 고정점이 없으면 믿을 수 없다.
  - 구독 주소는 브라우저가 보낸 값이다. 알려진 푸시 서비스 호스트로만 보낸다(SSRF). 테스트만 localhost 를 연다.
  - **로그아웃하면 이 기기의 구독을 뺀다.** 병동 폰은 돌려 쓴다. 같은 기기로 다른 사람이 등록하면 넘겨받는다.
  - 보내기는 커밋 뒤, 요청 스레드 밖에서 한다. 푸시 서비스가 느려도 접수 버튼은 바로 돌아와야 한다.
    `404`·`410` 은 죽은 구독이라 지운다.
  - VAPID 키가 없으면 조용히 꺼진다(공개키 `null` → 버튼을 감춘다). 키는 JWT 처럼 파일로 넘긴다.
  - **가짜 푸시 서비스로는 서명 모양을 못 잡는다.** 테스트의 localhost 서버는 본문을 풀어 보지만 VAPID 토큰은
    따지지 않았고, jjwt `audience().add()` 가 aud 를 배열로 적은 것을 실제 크롬 구독으로 보내 보고서야 FCM 403 으로 알았다.
    지금은 테스트가 aud 가 문자열인지 못 박고, 거절되면 푸시 서비스가 준 사유를 로그에 남긴다.
    실제 기기에 뜨는지는 HTTPS 배포 뒤 폰으로 봐야 한다 — Playwright 가 띄운 크롬은 푸시 수신을 끈다.
- **실시간 채널은 구독도 검사한다.** `StompAuthChannelInterceptor` 가 CONNECT 에서 토큰을 확인하고,
  SUBSCRIBE 는 토큰 소속의 `/topic/department/{id}` 만 허용한다. 거절은 ERROR 프레임 + 세션 종료다.
  한동안 CONNECT 만 보고 구독은 그냥 통과시켜서, 토큰 없이 붙어 아무 파트 채널이나 받을 수 있었다.
  이름을 방송에 싣지 않은 것은 원내망 밖의 브라우저 때문이지 로그인 안 한 사람 때문이 아니었다.
  - MockMvc 로는 못 잡는다. `RealtimeChannelApiTest` 가 포트를 열고 진짜 STOMP 로 붙는다.
    구독 등록은 내장 브로커가 영수증을 주지 않아 `SimpUserRegistry` 를 보고 기다린다.
  - 화면은 붙을 때마다 토큰을 다시 읽는다(`beforeConnect` → `freshAccessToken`). 접근 토큰이 30분이라
    처음 한 번만 박아 두면 와이파이가 한 번 끊긴 뒤부터 만료된 토큰으로만 재연결한다.
  - 메시지가 오면 무효화하는 쿼리 키 목록(`useRealtime`)은 화면 쿼리 키를 바꿀 때 같이 바꿔야 한다.
    V11 에서 병동 보드 키가 바뀐 뒤 옛 키(`encounters`)를 찌르고 있어 침대별 요청 수가 실시간으로 안 바뀌었다.
- **상태 이름은 전부 서버가 준다.** 목록·상세의 `statusLabel`, 진행 기록의 `toStatusLabel`, 실시간 방송의
  `toStatusLabel`. 화면에는 상태 이름표가 없다(`badges.tsx` 는 색만 정한다). 우선순위 이름만 화면이 든다 —
  종류와 무관하게 셋뿐이다.
- 알림 문구의 시각은 `ZoneId.systemDefault()` 로 바꿔 찍는다. 화면이 UTC 로 보내고 DB 도 UTC 로 돌려주므로
  `OffsetDateTime.format` 을 그대로 쓰면 15:30 예정이 06:30 예정으로 나간다. 테스트가 현지 시각을 못 박는다.
- 로그인은 같은 아이디로 10번 틀리면 15분 잠근다(`LoginAttemptLimiter`, Redis, `429 AUTH-004`).
  맞는 비밀번호도 같은 답이다 — 그래야 대입이 멈춘다. 없는 아이디도 센다(잠기는지로 계정 존재를 알 수 없게).
  잠긴 채 두드린 것은 감사 로그에 `LOCKED` 로 남는다.
- `startedAt` 은 처음 한 번만 찍는다. 부품대기에서 수리중으로 돌아올 때 덮어쓰면 수리에 걸린 시간이 줄어든다.
- 조회 조건은 `useUrlParam` 으로 주소에 담는다. 컴포넌트 안에만 두면 한 건을 열어 보고
  돌아왔을 때 기간과 검색어가 전부 처음으로 돌아간다. 링크로 넘길 수도 있게 된다.
  - `replace: true` 로 바꾼다. 조건을 고칠 때마다 방문 기록이 쌓이면 뒤로가기가
    이전 화면이 아니라 이전 검색 조건으로 돌아간다.
  - 입력 중인 글자는 넣지 않는다. 한 자 칠 때마다 조회가 나간다. 찾기를 누른 것만 올린다.
- 당겨서 새로고침(`usePullToRefresh`)은 병동 목록 세 화면에만 붙인다.
  앱으로 설치하면 브라우저 기본 동작이 사라져 새로고침할 방법이 아예 없어진다.
  - 맨 위(`scrollY <= 0`)에서 시작한 것만 센다. 목록 중간에서 쓸어내리는 것은 그냥 스크롤이다.
  - 당긴 거리는 **ref 로 들고 있는다.** 상태만 쓰면 손가락이 움직일 때마다 effect 가
    리스너를 떼었다 붙이고, `touchend` 가 아직 반영되지 않은 값을 보고 덜 당겼다고 판단한다.
  - 손가락만 받는다. 마우스는 F5 가 있고, 드래그로 글자를 고르는 것과 부딪힌다.
- 토스트는 `ToastProvider` 한 곳에서 그린다. 실시간 알림과 내 동작 확인이 같은 자리에 뜨므로
  따로 그리면 겹친다. 확인은 초록(success), 남이 한 일은 어두운색(info), 응급은 빨강(urgent).
  - **실시간 방송은 파트 채널로 나가 누른 사람에게도 되돌아온다.** `actorId` 로 걸러
    자기가 한 일을 자기에게 알리지 않는다. 안 그러면 "환자 출발 처리했습니다" 확인과
    "이송중 — 3병동 김간호" 알림이 동시에 뜬다. 알림함이 지키는 규칙과 같다.
    이름으로 거르지 않는 것은 동명이인 때문이다.
- 로딩은 `Skeleton` 으로 자리를 잡아 둔다. 목적은 예쁘게 기다리는 것이 아니라
  **밀림을 없애는 것**이다. 모양이 실제와 어긋나면 값이 들어오는 순간 화면이 밀려서
  "불러오는 중…" 한 줄보다 나을 것이 없다.
  - 제목 자리를 빼먹으면 안 된다. 값이 들어올 때 제목이 생기며 아래가 통째로 밀린다.
    통계 화면은 기간 입력칸까지 넣어야 높이가 맞는다.
  - `e2e/check-skeleton.mjs` 가 로딩 중과 로딩 후의 좌표를 재서 비교한다.
    눈으로 보면 이 정도 밀림은 놓친다.
  - 움직임을 줄여 달라고 설정한 사람에게는 깜빡이지 않는다(`motion-safe`).
- 사용 흐름: 조회 실패는 문구만 띄우지 않고 `LoadFailed` 로 다시 시도할 길을 준다.
  병원 와이파이는 자주 끊기는데 그때마다 앱을 껐다 켜게 만들면 안 된다.
  - 되돌릴 수 없는 전이(완료)에만 확인을 한 번 더 받는다. 전진 동작 전부에 확인을 붙이면
    한 손으로 쓰는 화면에서 오히려 방해가 된다.
  - 패널을 닫는 버튼에 "취소" 를 쓰지 않는다. 취소는 요청 취소라는 상태 이름이라
    "취소" 옆에 "요청 취소 확정" 이 놓이면 어느 쪽이 무엇인지 알 수 없다.
  - `e2e/check-ux.mjs` 가 확인한다. 오류 화면은 서버가 멀쩡하면 나오지 않고
    완료 확인은 요청을 끝까지 밀어야 나와서, 화면을 열어 보는 것만으로는 알 수 없다.
- 보안 헤더는 Caddy 가 붙인다. CSP 는 전부 `'self'` 다 — 외부에서 스크립트도 폰트도
  끌어오지 않기 때문이다. **헤더만 넣고 끝내면 안 된다.** 깨지는 CSP 는 없느니만 못하다.
  `e2e/check-csp.mjs` 가 스택을 실제로 올려 실시간 연결·서비스 워커·인라인 style 배치가
  살아 있는지 본다. CI 의 이미지 잡이 같은 검사를 돌린다.
- 보안: 토큰은 **RS256 키쌍**으로 서명한다. 대칭키면 토큰을 검증만 하면 되는 쪽도
  발급할 수 있는 키를 가져야 한다. 원내 게이트웨이는 검증만 해야 하므로 공개키만 준다.
  개인키가 없는 역할이 발급을 시도하면 조용히 넘어가지 않고 터진다.
  - 키는 환경변수가 아니라 **파일로** 넘긴다. 환경변수로 주면 `docker inspect` 와
    프로세스 목록에 개인키가 그대로 찍힌다.
  - `prod` 프로파일에서 `JwtKeyGuard` 가 개발용 키를 보면 기동을 막는다.
    `src/main/resources/keys/` 의 키쌍은 저장소에 공개돼 있어, 그대로 뜨면
    누구나 관리자 토큰을 위조할 수 있다. 화면은 멀쩡히 돌기 때문에 아무도 눈치채지 못한다.
  - 판별은 공개키 **지문**으로 한다. 파일 내용을 통째로 비교하면 줄바꿈 하나에 빗나간다.
    개발용 키를 새로 만들면 `DEV_PUBLIC_KEY_FINGERPRINT` 도 같이 고쳐야 하고,
    안 고치면 방어가 조용히 무력해진다. `JwtKeyGuardTest` 가 그 상수와 실제 파일을 붙들어 둔다.
- API 레벨 테스트: 권한 판정과 에러 코드는 컨트롤러·필터·예외 변환을 지나야 확정된다.
  서비스를 직접 부르는 테스트로는 `403 PERM-002` 가 실제로 그 값으로 나가는지 알 수 없다.
  MockMvc 로 진짜 JWT 를 받아 호출한다.
  - 통합 테스트는 `support/IntegrationTest` 를 상속한다.
  - **`@Testcontainers` / `@Container` 를 쓰지 않는다.** 그 조합은 컨테이너 수명을
    테스트 클래스 하나에 맞춰서, 먼저 끝난 클래스가 컨테이너를 내리고 캐시된 컨텍스트가
    죽은 DB 를 붙든다. static 블록에서 직접 띄우고 JVM 끝까지 살려 둔다.
  - 24시간 수정 창처럼 시간이 걸린 규칙은 기다리지 않고 `created_at` 을 SQL 로 되돌려
    확인한다. 규칙이 그 컬럼 하나만 보므로 이것으로 충분하다.
  - 간호기록에 삭제 엔드포인트가 생기지 않았는지도 테스트가 지킨다(405 를 기대한다).

### 구현된 API

| 메서드 | 경로 | 화면 |
|---|---|---|
| POST | `/auth/login` `/auth/refresh` `/auth/logout` | C-01 |
| GET | `/auth/me` | C-03 |
| GET | `/departments` `/service-items` | W-03, A-04 |
| GET | `/care-episodes` `/care-episodes/{ref}` | W-01, W-02 의 업무 쪽 절반 |
| POST GET | `/work-relations/active-subjects` `/episodes` `/messages` `/orders/{id}/message-refs` | 원내 서버가 업무 서버에 묻거나 알리는 곳. 화면은 부르지 않는다 (`WorkRelationPort`) |
| GET | `/phi/subjects/{ref}` `/{ref}/alerts` `/{ref}/checklist` `/subjects/search` | 사람 쪽 절반 |
| POST | `/phi/subjects/brief` | 목록 이름 채우기 |
| POST PATCH | `/phi/subjects/{ref}/alerts` `/phi/alerts/{id}/deactivate` | W-02 |
| POST | `/work-orders` | W-03 |
| GET | `/work-orders?direction=` | W-04, E-01 |
| GET | `/work-orders/{id}` `/{id}/events` | W-05, E-02 |
| POST | `/work-orders/{id}/transitions` | W-05, E-02 |
| GET | `/work-orders/{id}/messages` | W-05, E-02 — 누가·언제만 |
| POST GET | `/phi/work-orders/{id}/messages` `/messages/bodies` | W-05, E-02 — 대화 내용 (원내) |
| GET POST | `/phi/subjects/{ref}/vital-signs` | W-06 |
| GET POST | `/phi/subjects/{ref}/nursing-notes` | W-07 |
| PUT | `/phi/nursing-notes/{noteId}` | W-07 |
| GET PATCH POST | `/notifications` `/{id}/read` `/read-all` | C-02 |
| GET | `/stats/waiting-time` | A-01 |
| GET | `/phi/access-logs` | A-05 (원내) |
| GET | `/audit-logs` | A-05 업무 기록 탭 — 로그인·로그인 실패·기준 정보 변경. 환자 칸 없음 |
| GET POST PUT PATCH | `/staff` `/departments` `/service-items` (+ `/{id}/deactivate`) | A-02~04 |
| GET POST DELETE | `/push/public-key` `/push/subscriptions` | 폰 알림 켜기·끄기 (로그아웃 때 화면이 DELETE 를 부른다) |

목록 API 의 `size` 는 `Paging.of` 가 1~200 으로 눌러 담는다. 새 목록 엔드포인트도 `PageRequest.of` 를 직접 쓰지 않는다.

### 데모 계정

비밀번호는 전부 `nurse1234!` 다. 전부 가상 인물이다.

| 아이디 | 역할 | 소속 |
|---|---|---|
| `admin01` | ADMIN | 전산팀 |
| `head01` | HEAD_NURSE | 3병동 |
| `ward01` | NURSE | 3병동 |
| `ward02` | NURSE | 5병동 |
| `mri01` | NURSE | MRI실 |
| `ct01` | NURSE | CT실 |
| `lab01` | NURSE | 진단검사의학과 |
| `pharm01` | NURSE | 약제부 |
| `bme01` | NURSE | 의공학팀 |

## 최종 목표 (시연 시나리오)

폰(병동)과 노트북(MRI실)을 나란히 놓고 3분 안에:

1. 폰에서 이송 요청 등록 → 노트북 큐에 즉시 표시
2. 노트북 상세에 "인공관절 - MRI 금기 가능성" 경고 자동 표시
3. 접수 처리 → 폰 화면이 실시간 갱신
4. 창 두 개로 동시 처리 시도 → "다른 사용자가 먼저 처리했습니다"
5. 관리자 화면에서 평균 대기시간 통계 확인

이 시나리오가 끊김 없이 돌아가는 것이 완료 기준이다.
