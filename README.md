...
**작은 병원 반년치 데이터.** 화면을 실제 규모로 보고 싶으면 `SEED_PROFILE=hospital` 로 심는다.
병동 6개와 중환자실(228병상), 검사실 5곳, 진단검사의학과·약제부·의공학팀, 직원 170여 명이
지난 180일 동안 쓴 것처럼 채운다. 로컬에서 1분 정도 걸린다.

```bash
PG_VIA_DOCKER=nurse-collab-postgres SEED_PROFILE=hospital SEED_DIR=ops/demo-reset/seed-hospital node ops/demo-reset/reset.mjs
```

| 항목 | 대략 |
|---|---|
| 입원(재원 건) | 4,800건, 지금 재원 190여 명 |
| 업무 요청 | 10만 건 — 검체 절반, 약제 3할, 이송 1할, 의공 1천 건 |
| 진행 기록 | 60만 건 |
| 활력징후 · 간호기록 | 10만 건 · 12만 건 |
| 주의사항 · 메시지 · 알림(최근 하루) | 5천 건 · 1만 2천 건 · 7만 건 |

- 새벽 채혈, 낮 영상 검사, 투약 시간 전 약 요청, 퇴원약, 밤의 발열 배양처럼 병동의 하루 흐름을 따른다.
  진료과마다 나가는 요청이 다르다(신경과는 뇌 CT·MRI, 정형외과는 무릎 X-ray 와 마약성 진통제).
- 보류·취소·부품 대기가 섞여 있고, 최근 하루치 요청 일부는 진행 중으로 남는다.
- 데모 계정은 기본 데이터와 같다. 시나리오 문서(`docs/07`)는 기본 데이터 기준이라
  병실 번호와 환자는 이 데이터와 다르다.
- 요청은 API 가 아니라 SQL 로 넣는다. 10만 건을 API 로 만들 수는 없어서, 서버가 남기는 것과 같은
  모양(진행 기록·버전·요청번호 발번 표)으로 함께 만든다. 모든 값은 해시로 정해지고
  기준 시각 하나에 맞추므로 업무 DB 와 원내 DB 를 따로 채워도 같은 가명에 닿는다.
